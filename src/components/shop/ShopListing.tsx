...
export function ShopListing({ slug, type = "", concern = "" }: { slug: string; type?: string; concern?: string }) {
  ...
  const list = useMemo(() => {
    ...
    if (type) {
      next = next.filter(
        (p) =>
          p.subCategory.toLowerCase().replace(/\s+/g, "-").includes(type) ||
          type.includes(p.subCategory.toLowerCase().split(" ")[0]),
      );
    }
  ...
  const preview = hover
    ? products.filter((p) => {
        if (hover === "k-beauty") return p.origin === "k-beauty";
        if (hover === "j-beauty") return p.origin === "j-beauty";
        if (hover === "intl") return p.origin === "international";
        if (hover === "top") return p.isTopSelling;
        if (hover === "offer") return p.isTodayOffer;
        return true;
      })[0]
    : null;
  ...
  return (
    ...
    >
      ...
      <h1 className="font-display text-4xl sm:text-5xl capitalize mt-2 mb-6">{(type || slug).replace(/-/g, " ")}</h1>
        ...
        <aside className="lg:border-r lg:border-gold/20 lg:pr-5">
          ...
          {preview && (
            <div className="mt-3 overflow-hidden rounded-xl border border-gold/20 bg-white">
              ...
              <p className="p-2 text-[11px] text-off-black/60">{preview.brand}</p>
            </div>
          )}
