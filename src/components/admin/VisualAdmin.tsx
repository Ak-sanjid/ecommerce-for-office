  ...
  return (
    <div className="container-page py-10">
      ...
      <p className="max-w-2xl mt-4 text-off-black/70">
        Visual reorder writes to localStorage for instant preview. Server config, coupons, inventory,
        orders and recovery live at{" "}
        <a className="text-gold-dark underline" href="/admin">
          /admin
        </a>
        .
      </p>
      ...
      <ul className="space-y-2">
        {sorted.map((item) => (
          <li key={item.id} className="flex items-center gap-2 text-sm">
            ...
            <span className={`flex-1 ${item.visible ? "" : "line-through text-off-black/40"}`}>{item.label}</span>
